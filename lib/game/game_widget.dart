...
class GameWidget<G extends Game> extends StatefulWidget {
  /// The game instance in wich this widget will render
  final G game;
  ...
  ///
  /// o use it see [GameWidget.withOverlay]
  ...
  /// ```
  /// ...
  /// class MyGame extends Game with HasWidgetsOverlay {
  /// ...
  /// Widget build(BuildContext  context) {
  ///   return GameWidget(
  ///     game: MyGame(),
  ...
  GameWidget.withOverlay({
    ...
    @required this.overlayBuilderMap,
          ...
          """
          To use `GameWidget.withOverlay` you need to pass a instance of a game
          class mixed with `HasWidgetsOverlay`
          """,
        ),
        super(key: key);
...
class _GameWidgetState extends State<GameWidget> {
  ...
  void addOverlaysListener(Game game) {
    if (game is HasWidgetsOverlay) {
      final overlayGame = widget.game as HasWidgetsOverlay;
      overlayGame.overlays.addListener(onChangeActiveOverlays);
      activeOverlays = overlayGame.overlays.value;
    }
  }
  ...
  void removeOverlaysListener(Game game) {
    if (game is HasWidgetsOverlay) {
      final overlayGame = widget.game as HasWidgetsOverlay;
      overlayGame.overlays.removeListener(onChangeActiveOverlays);
    }
  }

  void onChangeActiveOverlays() {
    setState(() {
      activeOverlays = (widget.game as HasWidgetsOverlay).overlays.value;
    });
  }
  ...
  @override
  Widget build(BuildContext context) {
    Widget internalGameWidget = EmbeddedGameWidget(widget.game);
